...
def build_markdown(evaluation: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "## Backtest mở rộng theo thời gian",
            "",
            *backtest_rows,
            "",
            "## So sánh và chọn chiến lược",
            "",
            f"- Mô hình tốt nhất trên CV: `{selection.get('best_cv_model', 'N/A')}`",
            f"- Cách dự báo được chọn: `{selection.get('forecast_strategy', 'N/A')}`",
            f"- Cải thiện MAE so với Persistence: "
            f"**{format_percent(selection.get('mae_improvement_vs_persistence'))}**",
            f"- Kết quả tiêu chí chọn mô hình: **{selection.get('status', 'N/A')}**",
            "",
            *test_rows,
            "",
            "## Khoảng dự báo Conformal",
            "",
            f"- PICP trên test cuối: **{format_percent(conformal.get('picp'))}**",
